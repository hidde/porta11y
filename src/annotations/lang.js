 ...
 * @param {ReactComponent} [options.Component = props => <span lang={props[fieldName]}>{props.children}</span>] - Portable Text renderer
 */
function lang(options = {}) {
  ...
  const render =
    options.Component ||
    (props => <span lang={props[fieldName]}>{props.children}</span>)
